...
function getOffers(locale: Locale): Offer[] {
  ...
  const coupangSearch = `https://www.coupang.com/np/search?component=&q=${encodeURIComponent('ETF 입문서 투자')}`;

  // AliExpress search URL — fallback to trader desk gadget category
  const aliExpressSearch = 'https://www.aliexpress.com/w/wholesale-trading-monitor.html';

  const amazonUrl = new URL('https://www.amazon.com/s');
  amazonUrl.searchParams.set('k', 'etf investing book');
